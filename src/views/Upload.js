...
function Upload() {
  ...
  const allowedFileTypes = ['text/plain', 'image/jpeg', 'image/png', 'audio/mp3', 'video/mp4', 'application/pdf'];

  const handleFileChange = (event) => {
    const newFiles = Array.from(event.target.files);
    const validFiles = newFiles.filter(file => allowedFileTypes.includes(file.type));
    setFiles(validFiles);
    const rejected = newFiles.filter(file => !allowedFileTypes.includes(file.type));
        ...
        onDrop={handleDrop}>
          ...
          {files.length > 0 && (
              <ul id="gallery" className="flex flex-1 flex-wrap ">
                  {files.map((file) => (
                      <li key={file.name} className="h-full w-full text-center flex items-start gap-2">
                          <img
                              className="w-32 "
